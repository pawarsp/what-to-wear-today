...
def fill_missing_timestamps(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_new = df.copy()
    df_new = df_new.set_index("date", drop=True)
    full_index = pd.date_range(start=df_new.index.min(), end=df_new.index.max(), freq="h")

    # Detect the missing timestamps
    missing_timestamps = full_index.difference(df_new.index)

    # Define new indices for our df with the whol list of indices and
    # interpolate missing values
    df_new = df_new.reindex(full_index)
    ...
    return df_new.reset_index().rename(columns={"index": "date"})
